...
interface QuestionEditorProps {
  ...
  onDelete: () => void;
  /** Ouvert par défaut (ex. première question) */
...
export default function QuestionEditor({
  ...
  const preview = stripPreview(localQuestion.text).slice(0, 120);
    ...
    <div className="border border-gray-300 rounded-lg bg-gray-50 overflow-hidden">
      <div className="flex items-center justify-between gap-3 p-4">
        ...
        >
          ...
          {!expanded && preview ? (
            <p className="mt-1 text-sm text-gray-500 truncate pl-6">{preview}</p>
          ) : null}
        </button>
        <button
          type="button"
          onClick={onDelete}
          className="px-3 py-1 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors text-sm font-medium shrink-0"
        >
          Delete
        </button>
      </div>

      {/* Ne monter Quill que si la question est ouverte — évite le crash sur les longs quiz */}
      {expanded ? (
        <div className="space-y-4 px-4 pb-6 border-t border-gray-200 pt-4">
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <div>
                          ...
                          <RichTextEditor
                            ...
                            onChange={(value) =>
                              handleAnswerChange(answerIndex, { explanation: value })
                            }
                      ...
                      <div className="flex items-start space-x-3">
                        <div className="flex-1 space-y-2">
                          ...
                          <RichTextEditor
                            value={answer.text}
                            onChange={(value) =>
                              handleAnswerChange(answerIndex, { text: value })
                            }
